  ...
  const handleApply = () => {
    try {
      ...
      searchParams.forEach((value, key) => {
        if (key !== 'search') params.set(key, value);
      });

      if (state.selectedPropertyType && state.selectedPropertyType !== 'Tout') {
        params.set('type', state.selectedPropertyType.toLowerCase());
      }

      params.set('prix_min', state.priceRange[0].toString());
      params.set('prix_max', state.priceRange[1].toString());

      if (state.bedrooms !== null) {
        params.set('nombre_de_chambres', state.bedrooms.toString());
      }

      if (state.equipments.length > 0) {
        params.set('equipements', state.equipments
          .map(eq => `${eq.toLowerCase()}:true`)
          .join(','));
      }

      if (state.city) {
        params.set('region', normalizeRegion(state.city));
      }
      ...
      <AnimatePresence>
        {isFilterVisible && (
          <>
            ...
            <motion.div
              key="filter-content"
              {...(deviceType === "mobile" ? animationConfig.mobile : animationConfig.desktop)}
              className={`fixed bg-white shadow-xl z-50 ${
                deviceType === "mobile" ? 
                'top-0 right-0 h-full w-full max-w-xs' :
                deviceType === "tablet" ? 
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  ...
                  <div className="mb-6">
                    ...
                    <Slider
                      ...
                      onChange={(value) => setState(s => ({ ...s, priceRange: value as [number, number] }))}
                      trackStyle={{ backgroundColor: "#FC9B89" }}
                      railStyle={{ backgroundColor: "#E5E7EB" }}
                      handleStyle={{
                        backgroundColor: "#FC9B89",
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[1, 2, 3, 4, 5].map((num) => (
                        <motion.button
                          key={num}
                          whileHover={{ scale: 1.05 }}
                          onClick={() => setState(s => ({ ...s, bedrooms: s.bedrooms === num ? null : num }))}
                          className={`w-12 h-12 rounded-lg text-lg font-semibold transition-colors ${
                            state.bedrooms === num 
                              ? "bg-gradient-to-r from-[#FC9B89] to-[#FF6B6B] text-white" 
                              : "bg-[#014F86] text-white hover:bg-[#FC9B89]/80"
                  ...
                  <div className="mb-6">
                    <h3 className="text-lg font-semibold mb-4">Équipements</h3>
                    <div className="grid grid-cols-2 gap-3">
                      {["Climatisation", "Piscine", "Garage", "Chauffe-eau", "Ménagères", "Meubles", "Wifi"].map((equipment) => (
                        <label
                          key={equipment}
                          className="flex items-center gap-2 cursor-pointer"
                        >
                          <input
                            type="checkbox"
                            checked={state.equipments.includes(equipment)}
                            onChange={(e) => {
                              const newEquipments = e.target.checked
                                ? [...state.equipments, equipment]
                                : state.equipments.filter(e => e !== equipment);
                              setState(s => ({ ...s, equipments: newEquipments }));
                            }}
                            className="sr-only"
                          />
                          <div className={`w-5 h-5 border-2 rounded flex items-center justify-center 
                            ${state.equipments.includes(equipment) 
                              ? "bg-[#FC9B89] border-[#014F86]" 
                              : "bg-white border-[#014F86]"}`}>
                            {state.equipments.includes(equipment) && (
                              <svg className="w-3 h-3 text-white" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div className="mb-6">
                    <h3 className="text-lg font-semibold mb-4">Filtres actifs :</h3>
                    <div className="flex flex-wrap gap-2">
                      {state.selectedPropertyType && state.selectedPropertyType !== 'Tout' && (
                        <motion.span
                          initial={{ opacity: 0, scale: 0.5 }}
                          animate={{ opacity: 1, scale: 1 }}
                          className="bg-[#FC9B89]/20 text-[#014F86] px-3 py-1 rounded-full text-sm flex items-center gap-1 cursor-pointer hover:bg-[#FC9B89]/30 transition-colors"
                          onClick={() => setState(s => ({ ...s, selectedPropertyType: "" }))}
                        >
                      ...
                      {(state.priceRange[0] !== 50000 || state.priceRange[1] !== 1000000) && (
                        <motion.span
                          initial={{ opacity: 0, scale: 0.5 }}
                          animate={{ opacity: 1, scale: 1 }}
                          className="bg-[#FC9B89]/20 text-[#014F86] px-3 py-1 rounded-full text-sm flex items-center gap-1 cursor-pointer hover:bg-[#FC9B89]/30 transition-colors"
                          onClick={() => setState(s => ({ ...s, priceRange: [50000, 1000000] }))}
                        >
                          {state.priceRange[0].toLocaleString()} - {state.priceRange[1].toLocaleString()} FCFA
                          <IoClose className="text-[#014F86]" size={14} />
                        </motion.span>
                      )}
                      ...
                      {state.bedrooms !== null && (
                        <motion.span
                          ...
                          animate={{ opacity: 1, scale: 1 }}
                          className="bg-[#FC9B89]/20 text-[#014F86] px-3 py-1 rounded-full text-sm flex items-center gap-1 cursor-pointer hover:bg-[#FC9B89]/30 transition-colors"
                          onClick={() => setState(s => ({ ...s, bedrooms: null }))}
                        >
                          {state.bedrooms} chambre{state.bedrooms > 1 && 's'}
                          <IoClose className="text-[#014F86]" size={14} />
                        </motion.span>
                      )}
                      ...
                      {state.equipments.map((item) => (
                        <motion.span
                          ...
                          onClick={() => setState(s => ({ 
                            ...s, 
                            equipments: s.equipments.filter(e => e !== item) 
                          }))}
                        >
                          {item}
                          <IoClose className="text-[#014F86]" size={14} />
