...
function parseNaturalSearch(searchQuery: string): SearchFilters {
  ...
  const locationRegex = /(?:à|a|au|dans|sur|à la|aux|chez|près de|proche de|à côté de)\s+([^,0-9]+)/i;
  const locationMatch = remainingQuery.match(locationRegex);
  if (locationMatch) {
    filters.location = removeDiacritics(locationMatch[1].trim().replace(/[^a-z]/g, ''));
  ...
  const filteredData = mockLogements.filter(logement => {
    ...
    if (searchFilters.keywords.length > 0) {
      const logementText = removeDiacritics(
        `${logement.type} ${logement.titre} ${logement.quartier} ${logement.region}`
      ).toLowerCase();
